...
function _sync(localPath, remotePath, options, ignoreList) {
	...
	return Promise.coroutine(function *() {
		...
		var TIME_DRIFT = options.timeDrift || 30; // 30secs
			...
			return Promise.coroutine(function *() {
				switch (action) {
					...
					case sync_helper.REMOVE_REMOTE:
						l.vlog("Deleted " + remoteFile)
						remoteModified = true;
						yield sftp.unlinkP(remoteFile) 
						break;

					case sync_helper.REMOVE_LOCAL:
						l.vlog("Deleted local file " + path.relative(localPath,localFile));
		...
		return new Promise(function(resolve, reject) {
			...
			conn.on('ready', function() {
				...
				var p = Promise.coroutine(function *() {
					...
					var now = new Date();
					if (!options.skipTimeCheck && Math.abs(now-serverDate)/1000>TIME_DRIFT) {
					...
					if (Math.abs(now-serverDate)/1000<(TIME_DRIFT + MIN_TIME_DRIFT)) {
						// use a tighter timeDrift, down to 5 secs, if we can.
						TIME_DRIFT = Math.max(MIN_TIME_DRIFT, Math.abs(now-serverDate)/1000+MIN_TIME_DRIFT);
						l.log("Time drift set to "+MIN_TIME_DRIFT+"s")
					...
					if (changes || options.force) {
						...
						while (local.items.length) {
							...
							var s = sync_helper.determineSync(local, posix.relative(remotePath, remoteFile), undefined, lastSync, TIME_DRIFT);
