...
function __determineUsage(_this, context){ // this==FileInfo. Done like this to keep functionality private.
	...
	var chain = plugin_api.buildRenderChain(_this.path, context.config); 
	...
	if (!_this.isTheme && _this.isLayout)
		_this.relPath = path.join("_layouts", path.relative(context.getLayoutPath(), _this.path)); 
	...
	else if (_this.isTheme)
	{
		...
		*/
		...
		if (_this.isPartial || _this.isLayout) {
			...
			/* These checks now done in context.addFile
			...
			// TODO. Fix
			if (file_usage==USAGE.COPY && fs.fileExistsSync(path.join(context.getSourcePath(), _this.themeRelPath)))
				// already exists in the source folder. DON'T use it.
				file_usage = USAGE.IGNORE;
			*/
		}
...
function FileInfo(_path, stats, context) {
	...
	this.isLayout = this.isInDir(context.getLayoutsPath()) || this.isInDir(context.getThemeLayoutsPath());
	this.isPartial = this.isInDir(context.getPartialsPath()) || this.isInDir(context.getThemePartialsPath());
	this.isTheme = this.isInDir(context.getThemePath());
	...
	this.renderers = [];
	__determineUsage(this, context);
	l.logdIf(this, 2, "Fileinfo '" + this.relPath + "': isLayout("+this.isLayout+"), isPartial("+this.isPartial+"), isTheme("+this.isTheme+"), usage("+this.usage+")")
}
...
FileInfo.prototype.save = function(context) {
	...
	return Promise.coroutine(function *() {
		...
		if ((yield _shouldKeepExisting(_this.destPath, contents))) {
			l.log("Skipping unchanged '" + _this.relPath+"'")
			return false;
		}
