...
*/
	...
	for (var key in context.fields) {
		var source = context.fields[key];
		if (_.isArray(source)) {
			blocks[key] = _convertToItems(source);
			blocks.prefilter[key] = blocks.defaultPostsFilter;
		}
		else {

			blocks[key] = { }
			blocks.prefilter[key] = {};
			blocks.prefilter[key]['*'] = blocks.defaultListFilter;

			for (var subkey in source) {
				// subkey is eg. 'article', 'blog', or category 'some_category'
				
				blocks[key][subkey] = _convertToItems(source[subkey]);
				blocks.prefilter[key][subkey] = blocks.defaultPostsFilter;
			}
		}
	}
