...
function pluralise(s) { // not happy with this. it helps avoid name collisions tho
	var rules = [
		[/y$/, "ies"],
		[/blog$/, "blog"],
		[/([^s])$/, "$1s"],
...
*/
	...
	for (var key in context.fields) {
		...
		var key_s = pluralise(key);
		blocks[key_s] = { keys: Object.keys(context.fields[key])}
		...
		for (var posts in context.fields[key]) {
			...
			blocks[key_s][posts] = items;
			blocks[key_s][posts+".prefilter"] = blocks.defaultPostsFilter;
			//l.log("blocks["+key_s+"]["+posts+"] = " + l.dump(blocks[key_s][posts]))
