...
function _addOutputFile(dataname, name, single_list, context, fileInfo) {
...
function _collateData(fields, options, fileInfo, context) {
	...
	if (_should_make_html(options,'post_types', post_type )) {
		...
		_addOutputFile(data_name, as, true, context, fileInfo); // add output file of the name		
	}
	...
	for (var fieldname in options.fields) {
		...
		if (gen_html)
			_addOutputFile(data_name, as, false, context, fileInfo); // add output file for the container file (eg categories.html)
		...
		values.forEach(function(value) {
			...
			if (gen_html)
				_addOutputFile(data_name+'.'+value, as+'-'+value, true, context, fileInfo); // add output file (eg categories-some-category.html)
		})
	}
