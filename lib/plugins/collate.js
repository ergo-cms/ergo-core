...
*/
	console.log("list_type is: " + this.list_type)
...
function _addOutputFile(name, filename, single_list, context) {
	...
	if (filename.indexOf('.')<0)
		filename += '.'+plugin_api.DEF_EXTENSION;
	filename = path.join(path.dirname(filename), path.basename(filename, path.extname(filename))+'.'+plugin_api.DEF_EXTENSION);
