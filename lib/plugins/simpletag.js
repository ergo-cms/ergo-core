...
function __applyFilter(text, filterInf) {
	if (!filterInf || !_.isString(text))
		return text;
	if (filterInf.encode) {
		l.vlogd("Applying filter to: "+text)
		var t =  filterInf.encode.reduce(function(value, filterFn, param3) {
			l.vvlogd(l.dump(value) + "," + l.dump(filterFn) + ", " + l.dump(param3))
			return filterFn(value);
		}, text)
		l.vvlogd("Filtered text is: " +t)
		return t;
	}
	return text;
}
...
function __renderBlockList(fileInfo_list, template, defaultFields, filterInf, inf) { //text, params, list, filterInf, inf) {
	lif(inf, 0, '__renderBlockList')
	var texts = [];
	fileInfo_list.forEach(function(fi) {
		var fields = fi.fields;
		lif(inf, 2, "File info for '"+fi.relPath+"' : " + l.dump(fi))
...
function __dateFromFileInfo(fi) {
	var d = fi.fields['date'];
	if (!d) return null;
	try {
...
function __renderBlock(text, block_name, block_list, defaultFields, filterInf, inf) 
{
	...
	// convert the list from relPath to fileInfo
	block_list = block_list.map(function(relPath) {
		return inf.context.getFileInfoByRelPath(relPath);
	})

	// sort the files based on field info (ie date)
	block_list.sort(function(fi_a, fi_b) {
		var da = __dateFromFileInfo(fi_a);
		var db = __dateFromFileInfo(fi_b);
		if (da && db)
...
{ 
	...
	if (templated) {
		...
		// First though, make sure we render any tags in the 'content' 
		_fields.content = _render(_fields.content, inf.fields, {}, inf)
		fields.content = _fields.content;
		lif(inf, 2, "Finished rendering content for '"+fileInfo.relPath+"': " + _fields.content);
